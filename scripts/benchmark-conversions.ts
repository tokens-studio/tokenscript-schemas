...
async function runBenchmarks() {
  ...
  if (okhsl && okhsv && oklch && baseline) {
    const colors = 10000;
    console.log(`  sRGB → HSL (baseline): ${(baseline.avgMs * colors / 1000).toFixed(2)}s (${(baseline.avgMs * colors).toFixed(0)} ms)`);
    console.log(`  sRGB → OKLCH:          ${(oklch.avgMs * colors / 1000).toFixed(2)}s (${(oklch.avgMs * colors).toFixed(0)} ms)`);
    console.log(`  sRGB → OKHSL:          ${(okhsl.avgMs * colors / 1000).toFixed(2)}s (${(okhsl.avgMs * colors).toFixed(0)} ms)`);
    console.log(`  sRGB → OKHSV:          ${(okhsv.avgMs * colors / 1000).toFixed(2)}s (${(okhsv.avgMs * colors).toFixed(0)} ms)`);
